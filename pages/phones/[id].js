...
export async function getServerSideProps({ params }) {
  ...
  try {
    // Fetch the phone with seller info
    ...
    // Fetch related phones (same type first)
    const related = await prisma.phone.findMany({
      where: {
        id: { not: phone.id }
      },
      include: {
...
// ---------- Page ----------
export default function PhoneDetail({ item, related }) {
  ...
  return (
    <>
      <Head>
        <title>{item.title} • Phones • GT Store</title>
      ...
      <div className="min-h-screen bg-black text-white">
        ...
        <div className="mt-6">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-2 text-sm">
            ...
            {item.storage ? <Chip icon="hard-drive">{item.storage}</Chip> : null}
            <PiBadge />
          </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <section className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-sm p-6 sm:p-8">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-light text-white">Phone Details</h2>
              ...
              <div id="more-phone-details" className={`${detailsOpen ? 'mt-4' : 'hidden'}`}>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                  {/* Hardware specs */}
                  ...
                  <Fact label="Battery Capacity" value={item._meta?.batteryCapacity} />
                  <Fact label="Camera Specs" value={item._meta?.cameraSpecs} />
                  ...
                  <Fact label="Buttons" value={item._meta?.buttons} />
                  <Fact label="Water Damage" value={item._meta?.waterDamage} />
                  <Fact label="Repair History" value={item._meta?.repairHistory} />
                </div>

                {/* Connectivity */}
                {Array.isArray(item._meta?.connectivity) && item._meta.connectivity.length > 0 && (
                  <div className="mt-4">
                    <h3 className="text-sm font-semibold mb-2 text-white">Connectivity</h3>
                    <ul className="flex flex-wrap gap-2">
                      {item._meta.connectivity.map((c, i) => (
                        <li
                          key={i}
            ...
            <section className="rounded-2xl border border-white/20 bg-white/5 backdrop-blur-sm p-4 sm:p-5 shadow-soft">
              <h2 className="text-base font-semibold text-white">About this phone</h2>
          ...
          <aside className="lg:col-span-1 space-y-4">
            <section className="rounded-2xl border border-white/20 bg-white/5 backdrop-blur-sm p-4 sm:p-5 shadow-soft">
              ...
              <div className="mt-3 grid grid-cols-2 gap-2">
                ...
                <button
                  ...
                  className={`rounded-xl border border-white/20 bg-white/5 backdrop-blur-sm px-4 py-2 text-sm transition text-white hover:bg-white/10 ${
                    saved ? 'border-brand-blue text-brand-blue' : ''
                  }`}
                >
                  <Icon name={saved ? 'heart-solid' : 'heart'} className="h-4 w-4 inline mr-2" />
                  {saved ? 'Saved' : 'Save'}
                </button>
                <button onClick={onShare} className="rounded-xl border border-white/20 bg-white/5 backdrop-blur-sm px-4 py-2 text-sm text-white hover:bg-white/10">
            ...
            <section className="rounded-2xl border border-white/20 bg-white/5 backdrop-blur-sm p-4 sm:p-5 shadow-soft">
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <button className="col-span-2 rounded-xl border border-white/20 bg-white/5 backdrop-blur-sm px-3 py-2 text-white hover:bg-white/10">
                  Report Listing
                </button>
              </div>
            </section>
        ...
        {Array.isArray(related) && related.length > 0 && (
          <section className="mt-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <Link href="/phones" className="text-sm text-brand-blue hover:underline">
                View all
              </Link>
            </div>
            <Carousel>
              {related.map((p) => (
                ...
                >
                  <ListingCard item={p} href={`/phones/${p.id}`} />
                </div>
              ))}
            </Carousel>
          </section>
...
function Fact({ label, value }) {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm px-4 py-3 text-sm">
      ...
      <div className="font-medium text-white break-words mt-1">{value}</div>
    </div>
  )
}
